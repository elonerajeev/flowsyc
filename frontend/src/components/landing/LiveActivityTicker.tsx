...
export default function LiveActivityTicker() {
  ...
  return (
    <section id="activity" className="mx-auto max-w-6xl px-4 pb-4">
      <div
        className={`flex items-center justify-center gap-3 overflow-hidden transition-all duration-400 ${show ? "opacity-100 translate-y-0" : "opacity-0 translate-y-1"}`}
      >
        <div className="flex items-center gap-2 rounded-full border border-border/80 bg-muted/30 px-4 py-2 max-w-[85vw] truncate">
          ...
          <span className="text-xs text-muted-foreground">
            <span className="font-medium text-foreground/70">{event.text}</span>
            {" · "}
