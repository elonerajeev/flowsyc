...
function getStrength(password: string): { score: number; label: string; color: string } {
  ...
  if (score <= 1) return { score, label: "Weak", color: "bg-[#DC3545]" };
  if (score <= 3) return { score, label: "Fair", color: "bg-[#F0A030]" };
  if (score <= 4) return { score, label: "Good", color: "bg-[#5355D6]" };
  return { score, label: "Strong", color: "bg-[#2A8F7A]" };
}
  ...
  return (
    <AuthLayout showBack={false}>
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-4">
          <label className="space-y-2 text-sm font-semibold text-white/80">
            ...
            <input
              ...
              autoComplete="name"
              required
              className="h-11 w-full rounded-2xl border border-white/10 bg-white/[0.03] px-4 text-sm text-white outline-none transition focus:outline-none focus:ring-2 focus:ring-[#5355D6]/50 focus:ring-offset-2 focus:ring-offset-[#030308] disabled:cursor-not-allowed disabled:opacity-50"
            />
          </label>
          ...
          {password && (
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-medium text-white/40">{strength.label}</span>
              </div>
              <ul className="space-y-1">
                {strengthRules.map((rule) => (
                  <li key={rule.label} className="flex items-center gap-2 text-xs">
                    <span className={`h-1.5 w-1.5 rounded-full transition-colors ${rule.test(password) ? "bg-[#2A8F7A]" : "bg-white/10"}`} />
                    <span className={rule.test(password) ? "text-white/60" : "text-white/30"}>
                      {rule.label}
                    </span>
                  </li>
                ))}
        ...
        {error && (
          <p className="text-xs font-semibold text-[#DC3545]">{error}</p>
        )}
        ...
        <p className="text-center text-xs text-white/40">
          Already have an account? <button type="button" onClick={() => navigate("/login")} className="text-[#5355D6] underline">Sign in</button>.
        </p>
      </form>
    </AuthLayout>
  );
